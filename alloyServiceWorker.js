 ...
 * @throws {Error}
 */
var makeSendServiceWorkerTrackingData = async ({ xdm, actionLabel, applicationLaunches = 0 }, { logger, fetch }) => {
  const configData = await readFromIndexedDb(logger)
  const { browser, ecid, edgeDomain, edgeBasePath, datastreamId, datasetId } = configData || {}
 ...
 * @returns {Promise<void>}
 */
sw.addEventListener('push', (event) => serviceWorkerPushListener({ event, logger, sw }))
